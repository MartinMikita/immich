...
export class JobRepository {
  ...
  async clear(name: QueueName): Promise<void> {
    const workers = await this.db.selectFrom('graphile_worker.jobs').select('locked_by').distinct().execute();
    ...
    await this.workerUtils!.forceUnlockWorkers(workers.map((worker) => String(worker.locked_by)));
    ...
    await this.workerUtils!.completeJobs(jobs.map((job) => job.id));
  }
